# checks/hosts.nix
#
# `lib/hosts.nix` proven in both directions. Three cross-host checks, each shown to FIRE on a real
...
#     problem, not a cross-host collision, and reporting it here would blame the wrong layer)
...
let
  ...
  # it, and the check would fire on every incompletely-described host set -- the exact failure that makes a
  # cross-host check unadoptable.
  bothEmpty = assertHosts {
    hosts = { alpha = mkHost { }; beta = mkHost { }; };
  };

  # A host that is only a slug: no `resources` attribute at all.
  slugOnly = assertHosts {
  ...
  emptyFleet = assertHosts { hosts = { }; };

  firedWith = r: c: lib.any (v: v.check == c) r.violations;

  results = [
    (check "hosts/shared-disk-fires"
    ...
    (check "hosts/shared-disk-names-both-hosts"
    ...
    (check "hosts/two-empty-hosts-silent"
      bothEmpty.ok
      "two hosts that declare no disks and no addresses were reported as colliding -- `null` is being treated as a claimed value, which would make this check fire on every incompletely-described host set: ${bothEmpty.message}")

    (check "hosts/slug-only-host-silent"
      slugOnly.ok
    ...
    (check "hosts/self-duplicate-not-a-cross-host-violation"
      selfDuplicate.ok
    ...
    (check "hosts/lan-and-overlay-are-separate-namespaces"
      crossNamespace.ok
    ...
    (check "hosts/empty-hosts-is-ok"
      (emptyFleet.ok && emptyFleet.violations == [ ])
      "an empty host set did not come back clean")
  ];
in
results
